...
| `CB_MCP_LOG_ROTATION_MAX_SIZE`       | Global max size **in MB** per log file before it rotates, inherited by every level. `0` is invalid and falls back to the default with a startup warning    | `1` (1 MB)                                                     |
| `CB_MCP_LOG_MAX_BYTES`               | **Deprecated** — use `CB_MCP_LOG_ROTATION_MAX_SIZE` (MB). Global rotation size **in bytes**, still honored for backward compatibility; ignored when `CB_MCP_LOG_ROTATION_MAX_SIZE` is set | Unset                                     |
| `CB_MCP_LOG_ERROR_ROTATION_MAX_SIZE`   | Rotation size **in MB** for the ERROR log file; overrides `CB_MCP_LOG_ROTATION_MAX_SIZE` for ERROR                                                        | Inherits `CB_MCP_LOG_ROTATION_MAX_SIZE`                        |
| `CB_MCP_LOG_WARNING_ROTATION_MAX_SIZE` | Rotation size **in MB** for the WARNING log file; overrides `CB_MCP_LOG_ROTATION_MAX_SIZE` for WARNING                                                    | Inherits `CB_MCP_LOG_ROTATION_MAX_SIZE`                        |
| `CB_MCP_LOG_INFO_ROTATION_MAX_SIZE`    | Rotation size **in MB** for the INFO log file; overrides `CB_MCP_LOG_ROTATION_MAX_SIZE` for INFO                                                          | Inherits `CB_MCP_LOG_ROTATION_MAX_SIZE`                        |
| `CB_MCP_LOG_DEBUG_ROTATION_MAX_SIZE`   | Rotation size **in MB** for the DEBUG log file; overrides `CB_MCP_LOG_ROTATION_MAX_SIZE` for DEBUG                                                        | Inherits `CB_MCP_LOG_ROTATION_MAX_SIZE`                        |
...
- **Rotation & retention** — rotation size is configured **in MB** via `CB_MCP_LOG_ROTATION_MAX_SIZE` (global) and per-level `CB_MCP_LOG_<LEVEL>_ROTATION_MAX_SIZE` (inheriting the global); retention via `CB_MCP_LOG_RETENTION_BACKUP_COUNT` (global) and per-level `CB_MCP_LOG_<LEVEL>_RETENTION_BACKUP_COUNT`. A rotation size of `0` is invalid and falls back to the default with a startup warning. `CB_MCP_LOG_MAX_BYTES` (bytes) is deprecated but still honored for backward compatibility.
- **Environment snapshot** — with the `file` sink active, a one-shot system/environment record is written to a dedicated `mcp_server.env.log` file (derived from `CB_MCP_LOG_FILE`), regardless of log level and overwritten each start, so support always has the current environment even after other logs rotate.
