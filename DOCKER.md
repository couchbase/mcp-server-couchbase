...
| `CB_MCP_STATELESS_HTTP`                     | Handle each HTTP request with a fresh MCP transport instead of keeping per-session state. Only honored for `http`                                                                               | `true` when `CB_MCP_WORKERS` > 1, otherwise `false`            |
