...
| `CB_MCP_LOG_ERROR_ROTATION_MAX_SIZE`   | Rotation size **in MB** for the ERROR log file; overrides `CB_MCP_LOG_MAX_BYTES` for ERROR                                                              | Inherits `CB_MCP_LOG_MAX_BYTES`                                |
| `CB_MCP_LOG_WARNING_ROTATION_MAX_SIZE` | Rotation size **in MB** for the WARNING log file; overrides `CB_MCP_LOG_MAX_BYTES` for WARNING                                                          | Inherits `CB_MCP_LOG_MAX_BYTES`                                |
| `CB_MCP_LOG_INFO_ROTATION_MAX_SIZE`    | Rotation size **in MB** for the INFO log file; overrides `CB_MCP_LOG_MAX_BYTES` for INFO                                                                | Inherits `CB_MCP_LOG_MAX_BYTES`                                |
| `CB_MCP_LOG_DEBUG_ROTATION_MAX_SIZE`   | Rotation size **in MB** for the DEBUG log file; overrides `CB_MCP_LOG_MAX_BYTES` for DEBUG                                                              | Inherits `CB_MCP_LOG_MAX_BYTES`                                |
| `CB_MCP_LOG_RETENTION_BACKUP_COUNT`  | Rotated backups kept per per-level log file (excluding the live file), applied to every level unless overridden. `0` keeps only the live file            | `1`                                                            |
