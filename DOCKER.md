...
| `CB_MCP_LOG_MAX_BYTES`               | Global max size **in bytes** per log file before it rotates, inherited by every level. `0` is invalid and falls back to the default with a startup warning | `1048576` (1 MB)  |
| `CB_MCP_LOG_ERROR_ROTATION_MAX_SIZE`   | Rotation size **in bytes** for the ERROR log file; overrides `CB_MCP_LOG_MAX_BYTES` for ERROR                                                              | Inherits `CB_MCP_LOG_MAX_BYTES`                                |
| `CB_MCP_LOG_WARNING_ROTATION_MAX_SIZE` | Rotation size **in bytes** for the WARNING log file; overrides `CB_MCP_LOG_MAX_BYTES` for WARNING                                                          | Inherits `CB_MCP_LOG_MAX_BYTES`                                |
| `CB_MCP_LOG_INFO_ROTATION_MAX_SIZE`    | Rotation size **in bytes** for the INFO log file; overrides `CB_MCP_LOG_MAX_BYTES` for INFO                                                                | Inherits `CB_MCP_LOG_MAX_BYTES`                                |
| `CB_MCP_LOG_DEBUG_ROTATION_MAX_SIZE`   | Rotation size **in bytes** for the DEBUG log file; overrides `CB_MCP_LOG_MAX_BYTES` for DEBUG                                                              | Inherits `CB_MCP_LOG_MAX_BYTES`                                |
...
- **Rotation & retention** — per-level rotation size (`CB_MCP_LOG_<LEVEL>_ROTATION_MAX_SIZE`, inheriting `CB_MCP_LOG_MAX_BYTES`) and retention (`CB_MCP_LOG_<LEVEL>_RETENTION_BACKUP_COUNT`, inheriting `CB_MCP_LOG_RETENTION_BACKUP_COUNT`) are configurable per level in the table above. A rotation size of `0` is invalid and falls back to the default with a startup warning.
