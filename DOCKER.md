...
| `CB_MCP_CONNECTION_MODE`             | Which Couchbase service to connect to: `operational` or `analytics` (Enterprise Analytics). Mutually exclusive; only one mode's tools are loaded per process. `analytics` reuses `CB_CONNECTION_STRING`/`CB_USERNAME`/`CB_PASSWORD` but expects an `http(s)://` EA endpoint instead of `couchbase(s)://`, and is self-managed clusters only (rejected at startup against Capella). No Enterprise Analytics tools are implemented yet, so `analytics` mode currently loads zero tools. | `operational`                                                  |
