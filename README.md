...
| `CB_MCP_CONNECTION_MODE` | `--connection-mode` | Which Couchbase service the server connects to: `operational` (Data/Query/Index services) or `analytics` (Enterprise Analytics). Mutually exclusive — only one is active per server process, and only that mode's tools are loaded. `CB_CONNECTION_STRING`/`CB_USERNAME`/`CB_PASSWORD` are reused for either mode, but the connection string format differs: `couchbase(s)://host` for `operational`, `http(s)://host[:18095]` for `analytics`. `analytics` mode currently supports **self-managed clusters only** — the server refuses to start against a Capella connection string. No Enterprise Analytics tools are implemented yet, so `analytics` mode currently loads zero tools. | `operational` |
