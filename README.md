...
| `CB_MCP_LOG_ERROR_ROTATION_MAX_SIZE` | `--log-error-rotation-max-size` | Rotation size **in bytes** for the ERROR log file; overrides `CB_MCP_LOG_MAX_BYTES` for ERROR | Inherits `CB_MCP_LOG_MAX_BYTES` |
| `CB_MCP_LOG_WARNING_ROTATION_MAX_SIZE` | `--log-warning-rotation-max-size` | Rotation size **in bytes** for the WARNING log file; overrides `CB_MCP_LOG_MAX_BYTES` for WARNING | Inherits `CB_MCP_LOG_MAX_BYTES` |
| `CB_MCP_LOG_INFO_ROTATION_MAX_SIZE` | `--log-info-rotation-max-size` | Rotation size **in bytes** for the INFO log file; overrides `CB_MCP_LOG_MAX_BYTES` for INFO | Inherits `CB_MCP_LOG_MAX_BYTES` |
| `CB_MCP_LOG_DEBUG_ROTATION_MAX_SIZE` | `--log-debug-rotation-max-size` | Rotation size **in bytes** for the DEBUG log file; overrides `CB_MCP_LOG_MAX_BYTES` for DEBUG | Inherits `CB_MCP_LOG_MAX_BYTES` |
