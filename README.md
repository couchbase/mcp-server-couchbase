...
| `CB_MCP_LOG_ROTATION_MAX_SIZE` | `--log-rotation-max-size` | Global maximum size **in MB** per log file before it rotates, inherited by every level unless overridden. `0` is invalid and falls back to the default with a startup warning | `1` (1 MB) |
| `CB_MCP_LOG_MAX_BYTES` | `--log-max-bytes` | **Deprecated** — use `CB_MCP_LOG_ROTATION_MAX_SIZE` (MB). Global rotation size **in bytes**, still honored for backward compatibility; ignored when `CB_MCP_LOG_ROTATION_MAX_SIZE` is also set | Unset |
| `CB_MCP_LOG_ERROR_ROTATION_MAX_SIZE` | `--log-error-rotation-max-size` | Rotation size **in MB** for the ERROR log file; overrides `CB_MCP_LOG_ROTATION_MAX_SIZE` for ERROR | Inherits `CB_MCP_LOG_ROTATION_MAX_SIZE` |
| `CB_MCP_LOG_WARNING_ROTATION_MAX_SIZE` | `--log-warning-rotation-max-size` | Rotation size **in MB** for the WARNING log file; overrides `CB_MCP_LOG_ROTATION_MAX_SIZE` for WARNING | Inherits `CB_MCP_LOG_ROTATION_MAX_SIZE` |
| `CB_MCP_LOG_INFO_ROTATION_MAX_SIZE` | `--log-info-rotation-max-size` | Rotation size **in MB** for the INFO log file; overrides `CB_MCP_LOG_ROTATION_MAX_SIZE` for INFO | Inherits `CB_MCP_LOG_ROTATION_MAX_SIZE` |
| `CB_MCP_LOG_DEBUG_ROTATION_MAX_SIZE` | `--log-debug-rotation-max-size` | Rotation size **in MB** for the DEBUG log file; overrides `CB_MCP_LOG_ROTATION_MAX_SIZE` for DEBUG | Inherits `CB_MCP_LOG_ROTATION_MAX_SIZE` |
...
- **Environment snapshot** — when the `file` sink is active, a one-shot system/environment record (OS, Python, dependency versions, transport, resolved logging config, and redacted server config) is written to a dedicated `mcp_server.env.log` file (derived from the `CB_MCP_LOG_FILE` base). It is captured regardless of log level and overwritten on each start, so the current environment is always available for support and never scrolls out of a rotating log.
