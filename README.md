...
| `CB_MCP_LOG_LEVEL` | `--log-level` | Logging level for the MCP server: `off`, `trace`, `debug`, `info`, `warning`, `error` (see [Logging](#logging)) | `info` |
...
- **`CB_MCP_LOG_LEVEL`** — how much is logged: `info` (the default) logs lifecycle events and tool invocations, `debug` adds verbose internal detail, `trace` is the most verbose and additionally routes the Couchbase SDK's core logs through the server (the single most useful thing for diagnosing connectivity issues), and `off` disables all logging. With the file sink, trace records are written to the debug log file (there is no separate trace file). Use `trace` rather than the SDK's `PYCBC_LOG_LEVEL` env var, and don't set both.
