...
- **Environment snapshot** — when the `file` sink is active, a one-shot system/environment record (OS, Python, dependency versions, transport, resolved logging config, and redacted server config) is written to a dedicated `mcp_server.env.log` file (derived from the `CB_MCP_LOG_FILE` base). It is overwritten on each start, so support always has the current environment and it never scrolls out of a rotating log.
