...
def is_capella_connection_string(connection_string: str) -> bool:
    """True if every host in the connection string is under the Capella domain.

    Capella normally issues a single SRV hostname, but users may expand it to
    individual node hostnames when SRV lookup is unavailable — so this
    accepts any number of hosts as long as every one is under the Capella
    domain. Shared by `_determine_ssl_verification` (below) and the
    `connection_mode` startup guard in `mcp_server.py`, which refuses to
    start in Enterprise Analytics mode against a Capella connection string.
    """
...
def _determine_ssl_verification(
    ...
    is_capella_connection = is_capella_connection_string(connection_string)
