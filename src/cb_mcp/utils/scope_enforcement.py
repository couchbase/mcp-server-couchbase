...
tools (KV mutations and scope/collection management) only, and (deliberately)
neither scope alone unlocks the other —
...
def required_scopes_for_tool(
    ...
    *,
    ...
    Categorization rule (deliberately strict, matches the spec):
      - Names in ``write_tool_names`` (KV mutation tools and scope/collection
        management tools) require ``SCOPE_WRITE`` and ONLY ``SCOPE_WRITE``.
