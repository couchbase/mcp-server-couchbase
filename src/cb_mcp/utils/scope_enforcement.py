...
tools (KV mutations, scope/collection management, and index management) only,
and (deliberately) neither scope alone unlocks the other — SCOPE_WRITE on its
own cannot reach read tools or SQL++. This module wraps each tool with a scope
check that runs inside FastMCP's request context after token validation has
already populated the access token.
...
def required_scopes_for_tool(
    ...
    *,
    ...
    Categorization rule (deliberately strict, matches the spec):
      - Names in ``write_tool_names`` (KV mutation tools, scope/collection
        management tools, and index management tools) require ``SCOPE_WRITE``
        and ONLY ``SCOPE_WRITE``.
