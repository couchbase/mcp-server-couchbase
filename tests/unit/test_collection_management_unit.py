...
class TestCreateScope:
    async def test_creates_scope(self) -> None:
        ctx, cluster, cm = _make_ctx()
        with patch(_GET_CLUSTER, return_value=cluster, new_callable=AsyncMock):
            result = await create_scope(ctx, "b", "s")
    ...
    async def test_sdk_error_returns_error_envelope_not_raised(self) -> None:
        ctx, cluster, cm = _make_ctx()
...
class TestCreateCollection:
    async def test_creates_collection(self) -> None:
        ctx, cluster, cm = _make_ctx()
        with patch(_GET_CLUSTER, return_value=cluster, new_callable=AsyncMock):
            result = await create_collection(ctx, "b", "s", "c")
...
class TestDeleteScope:
    async def test_deletes_scope(self) -> None:
        ctx, cluster, cm = _make_ctx()
        with patch(_GET_CLUSTER, return_value=cluster, new_callable=AsyncMock):
            result = await delete_scope(ctx, "b", "s")
...
class TestDeleteCollection:
    async def test_deletes_collection(self) -> None:
        ctx, cluster, cm = _make_ctx()
        with patch(_GET_CLUSTER, return_value=cluster, new_callable=AsyncMock):
            result = await delete_collection(ctx, "b", "s", "c")
