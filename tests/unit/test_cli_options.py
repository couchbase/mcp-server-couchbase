...
def _resolve_oauth_kwargs(**overrides):
    """Minimal OAuth-enabled kwargs for ``_resolve_oauth`` (http + all JWT
...
class TestResolveOauthScopeLabelCollision:
    """``_resolve_oauth`` must reject read/write scope labels that collapse to
    ...
    def test_identical_custom_labels_rejected(self):
        with pytest.raises(click.UsageError, match="must be distinct"):
            mcp_server._resolve_oauth(
    ...
    def test_read_override_equal_to_write_canonical_rejected(self):
        ...
        with pytest.raises(click.UsageError, match="must be distinct"):
            mcp_server._resolve_oauth(**_resolve_oauth_kwargs(scope_read=SCOPE_WRITE))

    def test_distinct_labels_reach_build_oauth(self):
        sentinel = object()
        with patch("mcp_server.build_oauth", return_value=sentinel) as m:
            result = mcp_server._resolve_oauth(
    ...
    def test_defaults_do_not_collide(self):
        ...
        not trip the guard."""
        sentinel = object()
        with patch("mcp_server.build_oauth", return_value=sentinel):
            result = mcp_server._resolve_oauth(**_resolve_oauth_kwargs())
        assert result is sentinel
