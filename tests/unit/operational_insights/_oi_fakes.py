...
def make_oi_ctx(*, read_only_mode: bool = True) -> tuple[SimpleNamespace, MagicMock]:
    ...
    ``handle_registry`` is a real ``QueryResultsRegistry`` (not a further
    mock) — accessing it on a bare ``MagicMock`` would otherwise auto-vend a
    fake attribute, and the async query tools need real token minting/eviction.
    """
    cluster = MagicMock()
    provider = MagicMock()
    provider.get_cluster.return_value = cluster
    provider.handle_registry = QueryResultsRegistry()
