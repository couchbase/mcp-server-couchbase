"""Unit tests for Operational Insights index management tools.

Ported from the ``analytics-mcp`` prototype (branch
``DA-2027/Add-enterprise-tools``). Mocks the cluster's ``execute_query()`` so
these tests can verify CREATE INDEX statement construction and the
write-tool error envelope without a live Operational Insights cluster.

Identifiers are backtick-quoted, but input *validation* is still deliberately
absent from index.py, so there are no tests here for malformed input --
bad input either raises out of the formatter into the error envelope, or is
...
class TestCastDefault:
    ...
    def test_cast_follows_exclude_unknown_key(self) -> None:
        ...
        The server rejects this particular combination ("CAST modifier is
        only allowed for B-Tree indexes") for array indexes, but the
        ordering still has to be right for the plain-index case.
        """
        ctx, cluster = make_oi_ctx()
...
class TestCreateArrayIndex:
    ...
    def test_does_not_force_exclude_unknown_key(self) -> None:
        ...
        The server rejects it itself with a clear "Array indexes must
        specify EXCLUDE UNKNOWN KEY.", so the tool forwards the caller's
        statement rather than rewriting it.
        """
        ctx, cluster = make_oi_ctx()
