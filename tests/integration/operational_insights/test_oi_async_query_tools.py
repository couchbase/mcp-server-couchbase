...
Operational Insights cluster (see conftest.py in this directory for
connection details).

Ported from the ``analytics-mcp`` prototype (branch
``DA-2027/Add-enterprise-tools``).

Each test opens its own MCP session. That matters here in a way it does not
for the sync tools: query_handle tokens live in the server subprocess's
in-memory registry, so a token is only valid inside the session that minted
it.
"""
...
async def test_results_can_be_fetched_twice() -> None:
    """Fetching is a read, not a consume: the server keeps the buffers, so
    the same handle can be fetched again and returns identical rows."""
    async with create_oi_mcp_session() as session:
        handle = await _start_query(session, "SELECT 1 AS one")
...
async def test_cancel_refused_for_completed_query() -> None:
    ...
    The server answers a cancel for a completed query with a bare 404 and the
    SDK treats 404 as success, so without the tool's own status check this
    would report cancelled: true and evict the token.
    """
    async with create_oi_mcp_session() as session:
        handle = await _start_query(session, "SELECT 1 AS one")
